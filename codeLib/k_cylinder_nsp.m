...
for i = 1:size(omegaiM,1)
    ...
    if logFlag
        ...
        if ind2 < size(Spectrum.power_Spectrum,3);P_ki2 = 10*log10(squeeze(Spectrum.power_Spectrum(:,:,ind2)));else;P_ki2 = 0*P_ki;end
    else
        %
        ...
        if ind2 < size(Spectrum.power_Spectrum,3);P_ki2 = (squeeze(Spectrum.power_Spectrum(:,:,ind2)));else;P_ki2 = 0*P_ki;end
...
%     figure(201);
%     subplot(1,2,1);imagesc(P_ki);axis image;colorbar;
%     subplot(1,2,2);imagesc(P_k);colorbar;
%     drawnow;
%     pause;
end
