...
if numel(cDoppGuess)>1
    Ufit = fminsearch(@(U) 1-nsp_doppler_shift_extraction(Spectrum,P,wavenumbers(jj),U(1),U(2)),cDoppGuess,options);
else
    Ufit = fminsearch(@(U) 1-nsp_doppler_shift_extraction(Spectrum,P,wavenumbers(jj),U,0),cDoppGuess,options);
    Ufit(2) = 0;
end
